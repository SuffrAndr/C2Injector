...
MGFTPURL = "ftp://gdyd:XqUrM8Gos2Ck@175.6.15.95:21"
WEFTPURL = "ftp://gdydott:mangguo123@183.235.21.142:21/update/"
...
def getRecordFromDB():
    ...
    try:
        ...
        delFileSql = "SELECT A.ID, A.FILENAME,A.DOWNLOADSTATUS,A.STATUS,A.INITALTIME,A.FILESIZE FROM T_FILE A,T_CATEGORY B " \
            "WHERE A.ORIASSETID = B.ORIGINALID AND A.STATUS=6 OR A.STATUS=3 AND A.DOWNLOADSTATUS=2 AND A.ID>8570 AND A.ID < 27902 LIMIT 5"
        ...
        if not ONLYDEL:
            ...
            downSql = "SELECT A.ID, A.FILENAME,A.DOWNLOADSTATUS,A.STATUS,A.INITALTIME,A.FILESIZE FROM T_FILE A,T_CATEGORY B,T_CLIP C "\
                      ...
                      "AND A.FILEFORMATDESC >= 2 AND A.DOWNLOADSTATUS=1 AND A.ID>8570 AND A.ID < 27902 " \
